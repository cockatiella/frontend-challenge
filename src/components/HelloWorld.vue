...
function savePageState() {
  localStorage.setItem('pageState', JSON.stringify({
    searchedUsers: searchedUsers.value,
...
function loadPageState() {
  ...
  if (savedState) {
    const { searchedUsers: savedSearchedUsers, selectedUser: savedSelectedUser, filterText: savedFilterText, filterGender: savedFilterGender } = JSON.parse(savedState);
    searchedUsers.value = savedSearchedUsers;
...
watch([searchedUsers, selectedUser, filterText, filterGender], () => {
  savePageState();
});

async function fetchUsers(results: number) {
  const response = await fetch(`https://randomuser.me/api/?results=${results}&nat=us&seed=${username.value}`);
...
async function searchUser() {
  searchedUsers.value = await fetchUsers(resultsPerPage);
}

async function loadMoreResults() {
  loadingMore.value = true;
  const newResults = await fetchUsers(resultsPerPage);
  searchedUsers.value = searchedUsers.value.concat(newResults);
...
const filteredUsers = computed(() => {
  let filtered = searchedUsers.value;
...
<template>
  <div class="wrapper">
    ...
    <div class="user-details-card fixed" v-if="selectedUser">
      <div class="user-card">
        <div class="user-thumbnail">
